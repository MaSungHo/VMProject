...
public class ExistingVMController {
	...
	@ApiOperation(value = "특정 사용자에게 할당된 모든 VM 조회")
		@ApiImplicitParams({
			@ApiImplicitParam(name = "email", value = "VM 목록을 조회할 사용자의 이메일", required = true, dataType = "string", paramType = "path", defaultValue = ""),
		})
	@GetMapping("/existing/{email:.+}/email")
	public ResponseEntity<List<ExistingVM>> getVMsByEmail(@PathVariable String email) {
		return existingVMService.getVMsByEmail(email);
	}
	...
	@ApiOperation(value = "VM에 관한 정보 조회")
		@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "vmName", value = "정보 조회할 VM의 이름", required = true, dataType = "string", paramType = "path", defaultValue = ""),
		})
